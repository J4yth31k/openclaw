"""
Fundamental Analysis Agent for Forex Market
Monitors economic calendar, interest rates, and central bank bias.
...
class FundamentalAnalyst:
    ...
    def __init__(self):
        ...
        logger.info("Initializing FundamentalAnalyst")
    ...
    def analyze(self) -> Dict:
        ...
        """
        try:
            logger.info(f"Running fundamental analysis for {self.today}")
            ...
            logger.info(f"Fundamental analysis complete. Found {len(today_events)} events today, "
    ...
    def format_report(self, analysis: Dict) -> str:
        ...
        if "error" in analysis:
            return f"⚠️ *Fundamental Analysis Error*\n`{analysis['error']}`"
        ...
        if today_events:
            report_lines.append("🔔 *TODAY'S ECONOMIC EVENTS*")
            ...
            report_lines.append("✅ No high-impact events today\n")

        # Week events summary
        week_events = analysis.get("week_events", [])
        if week_events:
            report_lines.append("📈 *THIS WEEK'S KEY EVENTS*")
            event_summary = {}
            for event in week_events:
                if event.get("impact", 1) >= 4:
        ...
        report_lines.append("_Analysis generated by Fundamental Analyst Agent_")
