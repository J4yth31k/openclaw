"""
Trade ideas generator that synthesizes outputs from all analysis agents.

Combines technical, fundamental, sentiment, and correlation signals
into actionable trade setups with entry/exit levels and risk metrics.
"""
...
class TradeIdeaGenerator:
    """Generates trade ideas by scoring confluence of multiple signals."""
    ...
    def generate(self, technical: dict, fundamental: dict, sentiment: dict,
        ...
        """
        Generate trade ideas from confluence of all signals.

        Args:
            technical: Output from technical_analyzer
            fundamental: Output from fundamental_analyzer
            sentiment: Output from sentiment_analyzer
            correlation: Output from correlation_tracker

        Returns:
            Dictionary with trade ideas and metadata.
        """
        logger.info("Generating trade ideas from all signals")
        ...
        if events_24h:
            risk_warnings.append(
                ...
                "invalidation risk for all ideas"
            )
        ...
        if high_severity_divs:
            risk_warnings.append(
                f"⚠️ {len(high_severity_divs)} major correlation divergences - "
                "cross-asset risk"
            )
    ...
    def format_report(self, ideas: dict) -> str:
        """
        Format trade ideas for Telegram MarkdownV2.
        ...
        if ideas.get('status') != 'success':
            return f"❌ Trade idea generation failed"
        ...
        if not trade_ideas:
            lines.append("No trade ideas with sufficient confluence")
            lines.append("")
        else:
            lines.append(f"*Found {len(trade_ideas)} Trade Ideas:*")
            lines.append("")
            ...
            for i, idea in enumerate(trade_ideas[:5], 1):
                ...
                lines.append(f"{i}\\. {emoji} {pair} {direction} {conf_emoji} {confidence} conf")
                ...
                # Confluence
                lines.append(f"   Confluence: {confluence} | Reasoning: {idea['reasoning']}")
                lines.append("")

        # Summary
        lines.append("*Summary:*")
        lines.append(f"High Confidence: {ideas['high_confidence']}")
        lines.append(f"Medium Confidence: {ideas['medium_confidence']}")
