...
const AGENT_META: Record<string, { name: string; emoji: string }> = {
  ...
  fury:           { name: 'Nick Fury',       emoji: '🎯' },
  ironman:        { name: 'Iron Man',        emoji: '🦾' },
  captain:        { name: 'Capt. America',   emoji: '🛡️' },
  scarlet:        { name: 'Scarlet Witch',   emoji: '🔮' },
  vision:         { name: 'Vision',          emoji: '👁️' },
  thor:           { name: 'Thor',            emoji: '⚡' },
  widow:          { name: 'Black Widow',     emoji: '🕷️' },
  spiderman:      { name: 'Spider-Man',      emoji: '🕸️' },
  hawkeye:        { name: 'Hawkeye',         emoji: '🏹' },
  strange:        { name: 'Dr. Strange',     emoji: '🔯' },
...
function quickPrompts(agentId: string): string[] {
  const map: Record<string, string[]> = {
    hulk:           ['Any setups to backtest?', 'What\'s the win rate looking like?', 'Review my R:R'],
    fury:           ['What\'s the priority?', 'Status report', 'Where are we falling short?'],
    ironman:        ['What\'s the tech setup?', 'EMA or VWAP bias?', 'System optimization?'],
    captain:        ['Macro outlook?', 'Any high-impact news?', 'What\'s the fundamental bias?'],
    scarlet:        ['What\'s market sentiment?', 'Fear or greed right now?', 'COT positioning?'],
    vision:         ['Order flow reading?', 'Volume profile update', 'Delta divergence?'],
    thor:           ['DXY correlation?', 'Cross-asset check', 'Any correlation trades?'],
    widow:          ['Best signal right now?', 'Entry conditions?', 'Risk/reward on next trade?'],
    spiderman:      ['Any big news today?', 'Calendar events?', 'What\'s moving the market?'],
    hawkeye:        ['Webhook working?', 'Alert setup help', 'Signal routing check'],
    strange:        ['Position size check', 'Max risk today?', 'Drawdown scenario?'],
