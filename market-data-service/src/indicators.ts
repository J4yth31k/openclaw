 ...
 * from live 1-minute bar data provided by the active MarketDataProvider.
 */

import { Bar } from './types';
import { getProvider } from './shared';

export interface ScalpingSnapshot {
  symbol:      string;
  price:       number;
  ema8:        number;
  ema13:       number;
...
export async function computeScalpingSnapshot(symbol: string): Promise<ScalpingSnapshot | null> {
  ...
  try {
    ...
    const from = now - 60 * 250; // ~250 1-minute bars
    const bars = await provider.getBars(symbol, '1', from, now);
    ...
    // Simplified D = same as K for single-bar (no history of K here)
    const stochD = stochK;
    ...
    // ── ATR ───────────────────────────────────────────────────────
    const atrVals = atrArr(bars, 14);
    const atrLast = atrVals[n];

    // ── TTM Squeeze ───────────────────────────────────────────────
    const sqPeriod = 20;
    const bbMult = 2.0, kcMult = 1.5;
    const smaVals = smaArr(closes, sqPeriod);
    const sdVals  = stddevArr(closes, sqPeriod);
    const ema20s  = emaArr(closes, sqPeriod);
    const atr20s  = atrArr(bars, sqPeriod);

    const bbU = smaVals[n] + bbMult * sdVals[n];
    const bbL = smaVals[n] - bbMult * sdVals[n];
    const kcU = ema20s[n] + kcMult * atr20s[n];
    const kcL = ema20s[n] - kcMult * atr20s[n];
    const squeezeOn = bbL > kcL && bbU < kcU;

    // Momentum (LazyBear)
    const momDeltas: number[] = [];
    for (let i = sqPeriod; i < closes.length; i++) {
      const hh = Math.max(...highs.slice(Math.max(0, i - sqPeriod + 1), i + 1));
      const ll = Math.min(...lows.slice(Math.max(0, i - sqPeriod + 1), i + 1));
      momDeltas.push(closes[i] - ((hh + ll) / 2 + smaVals[i]) / 2);
    }
    const momentum = linregLast(momDeltas, sqPeriod);
